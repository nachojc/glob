...
import {EnvBranchLocatorEndPointModel, EnvBranchLocatorModel} from '../../models/env-branch-locator.model';
...
export class ConfigurationService {
  ...
  constructor(
    ...
    this.activatedRoute.queryParams.pipe(first()).subscribe(
      params => {
        ...
        this.geoPosition.getCurrentPosition().pipe(first()).subscribe(
  ...
  private fetchRemoteConfig(baseEndpoint, viewType) {
    ...
    this.http.get<any>(configEndpoint).pipe(
  ...
  resolveConfigUrl(pos?: Position): string {
    ...
    if (!endpoints || !endpoints.length) {
      throw new Error('No branch locator configuration endpoints have been defined.');
    }
    ...
    const nearestEndpoint = this.branchLocatorEnv.endpoints.reduce(
        ( resultEndpoint,
          thisEndpoint ): EnvBranchLocatorEndPointModel => {
            if (!resultEndpoint) {
              return thisEndpoint;
            }
            ...
            return (thisEndpointDistance < resultEndpointDistance ? thisEndpoint : resultEndpoint);
        }
      );
